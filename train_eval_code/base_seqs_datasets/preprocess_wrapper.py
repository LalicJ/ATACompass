...
# from data.hg_map import hg_dict
hg_dict = defaultdict(lambda : "hg19")
...
def train_val_preprocess_function_wrapper(
        fasta_files, num_sequences, max_jianji_per_sequence, 
        rc_aug=False, add_eos=False, model_name="pretrained/gpt2"):
    # 获取人类基因组中各染色质的完整碱基序        
    chr_seq = {}
        ...
        with open(fasta_file) as fa:
            ...
            for index, chr in enumerate(chrs):
                ...
                if re.match("^chr[A-Za-z0-9]*$", chr_name):
                    cur_chr_seq[chr_name] = base_seqs[index + 1].replace("\n", "")
        name = fasta_file.split('/')[-1].split('.')[0]
        chr_seq[name] = cur_chr_seq
    # with open(fasta_file) as fa:
    #     with open(fasta_file) as fa:
    #         s = fa.read() #.replace("\n", "")
    #         chrs = re.findall(">chr[A-Za-z0-9]*\n", s)
    #         base_seqs = re.split(">chr[A-Za-z0-9]*\n", s, maxsplit=0, flags=0)

    #         for index, chr in enumerate(chrs):
    #             chr_name = chr.replace(">", "").replace("\n", "")
    #             if chr_name not in chr_seq:
    #                 chr_seq[chr_name] = base_seqs[index + 1].replace("\n", "")
    #             elif re.match('N+', chr_seq[chr_name]):
    #                 chr_seq[chr_name] = base_seqs[index + 1].replace("\n", "")
            # elif re.match('N+', chr_seq[chr_name]):
            #     chr_seq[chr_name] = base_seqs[index + 1]
    # with open(fasta_file) as fa:
    #     for line in fa:
    #         line = line.replace('\n','')
    #         if line.startswith('>'):
    ...
    def get_train_sequence(order, file_name):
        # if order == "chr21-46769620-46770120":
        #     print("breakpoint")
        ...
        hg_name = hg_dict[file_name]
        chromosome = chr_seq[hg_name][chr_name]
        ...
        if start >= len(chromosome) or end <= 0:
            start = 0
            end = 0
            left_padding = 0
            right_padding = max_jianji_per_sequence
            print(f'Warning: start ({start}) or end ({end}) is out of range for chromosome {order}.')
        ...
        assert len(seq) == max_jianji_per_sequence, f"{order} error" 
        if rc_aug and coin_flip():
            seq = string_reverse_complement(seq)
        return seq
    
    # 根据碱基序列的起始位置从染色质中按序选取相应的碱基片段
    def get_val_sequence(order, file_name):
        [chr_name, start, end] = order.split('-')
        # chr_name, start, end = "chr1", 4, 20
        start = int(start)
        end = int(end)
        ...
        seq = 'N' * left_padding + chromosome[start:end] + 'N' * right_padding
        # assert len(seq) == max_jianji_per_sequence, f'Length of sequence ({len(seq)}) from interval ({start}, {end}) of {chromosome} (len={len(chromosome)}) is not equal to `max_jianji_per_sequence` ({max_jianji_per_sequence})'
        assert len(seq) == max_jianji_per_sequence
        return seq
    ...
    def train_preprocess_function(examples):
        ...
        for i in range(batch_size):
            ...
            input_ids_of_seqs = []            
            for order in orders:
                seq = get_train_sequence(order, examples["file_name"][i])
        ...
        # 获取 LLM 所需的模型输入
        model_inputs = {"input_ids": [], "attention_mask": [], "target_ids": [], \
                        "start_index_of_seq": [], "input_ids_of_seqs": batch_input_ids_of_seqs}
        batch_input_ids_of_pre_prompts = prompt_tokenizer(batch_prefix_inputs)["input_ids"]
        batch_input_ids_of_post_prompts = prompt_tokenizer(batch_suffix_inputs)["input_ids"]
        batch_input_ids_of_targets = prompt_tokenizer(batch_targets)["input_ids"]        
        for i in range(batch_size):
            input_ids_of_pre_prompt = batch_input_ids_of_pre_prompts[i][:-1]
            input_ids_of_post_prompt = batch_input_ids_of_post_prompts[i][:-1]     
            input_ids_of_target = batch_input_ids_of_targets[i] + [prompt_tokenizer.eos_token_id]

            start_index_of_seq = len(input_ids_of_pre_prompt)
            model_inputs["start_index_of_seq"].append(start_index_of_seq)

            input_ids = input_ids_of_pre_prompt + [prompt_tokenizer.eos_token_id] * num_sequences \
                        + input_ids_of_post_prompt + input_ids_of_target
            model_inputs["input_ids"].append(input_ids)

            target_ids = [-100] * (len(input_ids_of_pre_prompt) + num_sequences + len(input_ids_of_post_prompt)) \
                            + input_ids_of_target  # [-100]*len(question)+answer
            model_inputs["target_ids"].append(target_ids)      
            
            attention_mask = [1] * len(input_ids) # attention mask the whole sentence     
    ...
    def val_preprocess_function(examples):
        ...
        for i in range(batch_size):
            ...
            input_ids_of_seqs = []            
            for order in orders:
                seq = get_val_sequence(order, examples["file_name"][i])
                seq = jianji_seq_tokenizer(seq, add_special_tokens=False)  # add cls and eos token (+2)
                input_id_of_seq = [jianji_seq_tokenizer.cls_token_id] + seq["input_ids"]  # get input_ids
                # need to handle eos here
                if add_eos:
...
def test_preprocess_function_wrapper(fasta_files, num_sequences, max_jianji_per_sequence, \
                                     rc_aug=False, add_eos=False, model_name="pretrained/gpt2"):
    # 获取人类基因组中各染色质的完整碱基序        
    chr_seq = {}
    for fasta_file in fasta_files:
    ...
    # 根据碱基序列的起始位置从染色质中按序选取相应的碱基片段
    def get_test_sequence(order, file_name):
        [chr_name, start, end] = order.split('-')
        start = int(start)
        end = int(end)
        # chr_name, start, end = "chr1", 4, 20
    ...
    def test_preprocess_function(examples):
        ...
        for i in range(batch_size):
            ...
            input_ids_of_seqs = []            
            for order in orders:
                seq = get_test_sequence(order, examples["file_name"][i])
                seq = jianji_seq_tokenizer(seq, add_special_tokens=False)  # add cls and eos token (+2)
                input_id_of_seq = [jianji_seq_tokenizer.cls_token_id] + seq["input_ids"]  # get input_ids
                # need to handle eos here
                if add_eos:
